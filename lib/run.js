(function () {
  ...
  /**
   * @param waterMarkName string
   * @param canvasOpt object
   * @param warStyle object
   */
  const createWaterMark = (waterMarkName, canvasOpt = {}, warStyle = {}) => {
    ...
    ctx.font = `${canvasOpt?.fontSize || "16px"} ${
      canvasOpt?.fontFamily || fontFamily
    }`;
    ...
    waterMarkDOM.style.cssText += `
            ...
            background-position: ${canvas.width / 1.5}px ${
      canvas.height / 1.4
    }px;
    ...
    if (!getIdEle(waterMarkDOM.id)) {
      $body ? $body.appendChild(fragment) : console.error("水印加载失败");
    }

    const resetDom = (targetNode) => {
      targetNode.remove();
      createWaterMark(waterMarkName, canvasOpt, warStyle);
    };
    ...
    const mutationCallback = (mutationList) => {
      for (let mutation of mutationList) {
        ...
        if (
          mutation.oldValue === "water-mark" ||
          mutation.target.id === "water-mark"
        ) {
          resetDom(targetNode);
          break;
        }
  ...
  window.canvansWatermark = createWaterMark;
  window.unObserver = unObserver;
})();
